...
class Users extends Controller
{
    ...
    function registerT(){
        ...
        filter_var($_POST['mail'],FILTER_VALIDATE_EMAIL) === false) {
            ...
            session()->put('error',"Mots de passe differents");
            return redirect('/register');
        }else {
            if (UsersDB::where('email', '=', $_POST['mail'])->exists()) {
                session()->put('error',"Email déjà utilisé");
                return redirect('/register');
            }
            $u= new UsersDB();
            $u->nom =  $_POST['Nom'];
    ...
    public function desinscription(){
        if(session('id')){
            $id = session('id');
            UsersDB::where('id',$id)->update(['qrcodetoken'=>NULL,'DateGenQrCode'=>NULL]);
            return redirect('/');
        }
    }
